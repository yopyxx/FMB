...
// 그룹(소령/중령) 내 모든 유저의 totalAdmin/totalExtra를 daily에서 재계산(불일치 방지)
...
// "현재 주(일요일 시작)"는 유지하고, 그 직전 7일치 daily 기록을 전부 삭제
function clearPrevious7DaysBeforeCurrentWeek(group) {
  const today = getReportDate();
  const currentWeekStart = getSundayWeekStart(today);     // 이번주 일요일
  const rangeStart = addDays(currentWeekStart, -7);       // 지난주 일요일
  const rangeEnd = currentWeekStart;                      // 이번주 일요일(미포함)
  ...
  return { rangeStart, rangeEnd, clearedEntries };
}
...
client.on('interactionCreate', async interaction => {
  ...
  // ✅ 수동 주간 초기화: "일요일 02시 이후(=이번주)"는 유지 / 그 이전 7일(지난주) 점수 원천(daily) 삭제
  if (cmd === '초기화주간') {
    const today = getReportDate();
    const thisWeekStart = getSundayWeekStart(today);
    const lastWeekStart = addDays(thisWeekStart, -7);
    ...
    // ✅ 실제 점수 초기화(지난주 7일 daily 삭제)
    const majRes = clearPrevious7DaysBeforeCurrentWeek(data.소령);
    const ltRes = clearPrevious7DaysBeforeCurrentWeek(data.중령);
    ...
    return interaction.reply({
      content:
        ...
        `- 유지(이번 주): ${thisWeekStart} ~ 현재\n` +
        `- 삭제(지난주 7일): ${majRes.rangeStart} ~ ${addDays(majRes.rangeEnd, -1)}\n` +
        `- 삭제된 daily 항목 수: 소령 ${majRes.clearedEntries} / 중령 ${ltRes.clearedEntries}\n` +
        `※ 지난주 스냅샷은 weekly history에 저장되었습니다.`,
      ephemeral: false
    });
  }
