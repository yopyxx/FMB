...
// 요구사항:
// - 보호: "해당 주 일요일 02시 이후"
// - 삭제: "그 이전 7일 점수" (= reportDate 7일치)
// 이 시스템은 일자키(YYYY-MM-DD)만 저장하므로,
// "일요일 02시 이전(00:00~01:59)"에 들어온 보고는 getReportDate()에 의해 전날 키로 저장됨.
// 따라서 '삭제 7일치'는 "이번 주 시작(일요일 date key) 이전 7개의 reportDate"를 지우면 정확히 충족됨.
...
client.on('interactionCreate', async interaction => {
  ...
  // ✅ /초기화주간:
  // - 보호: "해당 주 일요일 02시 이후" (즉, 이번 주 일요일 dateKey부터는 유지)
  // - 삭제: 그 이전 7일치(reportDate 7일) daily 기록 삭제
  // - 일요일 02시 이전에 들어온 보고는 reportDate가 전날로 저장되므로 자연스럽게 삭제 범위에 포함됨
  if (cmd === '초기화주간') {
...
[1] /초기화주간이 점수를 실제로 초기화하지 않던 원인
- 기존 코드는 weekStart만 바꿔서 "표시 범위"만 변하고,
  원자료(users[uid].daily[date])가 남아서 점수가 계속 계산됨.
- 현재 코드는 daily 기록을 실제 삭제하므로 점수가 확실히 초기화됨.

[2] "일요일 02시 이전" 처리
- 이 구조는 시간 단위 저장이 없고 "YYYY-MM-DD" 날짜키만 있음.
- getReportDate()가 새벽 2시 이전이면 전날로 보정하므로,
  일요일 00:00~01:59 보고는 전날 키로 저장됨(= 삭제 구간에 자연 포함).

[3] 일요일 계산 버그 방지
- KST 00:00을 UTC로 해석할 때 날짜가 밀려 weekStart가 월요일로 나오는 사례가 있음.
- 정오(12:00 +09:00) 기준 getUTCDay()로 요일을 구해 밀림을 방지함.

[4] 누적(totalAdmin/totalExtra) 정합성
- daily 삭제/초기화 후 누적이 남아있으면 통계가 틀어짐.
- recomputeTotals(group)로 항상 daily 합계로 재계산하여 정합성을 보장함.

[5] 주간 자동 초기화
- 매주 일요일 02:00으로 설정(요구 기준과 일치)
- 자동은 스냅샷/범위 갱신용. 실제 삭제는 수동 /초기화주간에서만(운영상 안전)

[6] 퍼센타일 표기 주의
- pct = ceil(rank/n*100) 방식이라 인원이 적으면 1등도 34% 등으로 표기될 수 있음.
- "상위 X%" 정의를 바꾸고 싶으면 퍼센타일 정의를 재설계 필요.
*/