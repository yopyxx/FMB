...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildMembers, // ✅ 역할 보유 전체 인원 포함/제외 필터링에 필요
 ...
 *   rankName: '소령'|'중령',
 *   mode: 'today'|'yesterday'|'week'|'lastweek',
 *   key: 'YYYY-MM-DD', // daily: dateStr / weekly: weekStart
 *   list: [{...}],     // 표시용 전체 리스트(정렬된)
 *   pageSize: 28
 * }
 */
...
function getDayTotalsOnly(rankName, dateStr) {
  ...
  // NOTE: 캐시는 "보고 데이터 있는 유저들" 기준으로 만들지만,
  // 멤버 전체 합산 시 totalsMap.get(uid) || 0 처리하면 동일(보고 안한 인원은 어차피 0점/eligible 제외)
...
function makeDailySnapshot(rankName, dateStr) {
  // 기존 스냅샷은 "보고 데이터 기준"이지만,
  // 새 요구사항(전체 역할 보유자 포함) 표시는 명령 실행 시점에 동적으로 계산한다.
  // 이 함수는 자동 스냅샷 보관 용도로 유지.
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isButton()) {
    ...
    if (!customId.startsWith('pg|')) return;

    // 권한 체크: 감독관만 페이지 넘김 허용
    ...
    if (!session || session.rankName !== rankName || session.mode !== mode || session.key !== key) {
      ...
      if (mode === 'today' || mode === 'yesterday') {
        ...
        for (const uid of memberIds) {
          totals[uid] = { nick: group.users?.[uid]?.nick || `<@${uid}>`, weeklyTotal: 0 };
        }
        for (const d of weekDates) {
          const totalsMap = getDayTotalsOnly(rankName, d);
          for (const uid of memberIds) {
        ...
        const list = Object.entries(totals)
          .map(([uid, v]) => ({ userId: uid, nick: v.nick, weeklyTotal: v.weeklyTotal }))
  ...
  if (cmd === '어제점수') {
    // 공용 명령은 기존 스타일 유지(원하면 이것도 페이지네이션/전체포함으로 동일하게 확장 가능)
    const dateStr = getYesterdayDate();

    const embed = new EmbedBuilder()
      .setTitle(`어제 점수 (기준일: ${dateStr})`)
      .setDescription('※ 공용 명령은 기존 유지 상태입니다. (요청 시 동일 규칙+페이지네이션 적용 가능)');
...
[1] 표시 범위 확대(8개 명령어)
- 기존: data.users(보고한 사람) 중심
- 변경: 서버의 "해당 역할 보유자 전체"를 guild.members.fetch()로 가져와서 모두 포함
  - 소령: MAJOR_ROLE_ID 보유자 전체
  - 중령: LTCOL_ROLE_ID 보유자 전체
- 제외: EXCLUDED_ROLE_IDS(감독관/사령본부/인사행정부단장) 중 하나라도 있으면 제외
- 보고 미제출자는 admin=0/extra=0 → 최소업무 미달 → 0점으로 표시

[2] 페이지네이션(8개 명령어)
- 한 페이지 28명
- 이전/다음 버튼으로 페이지 이동
- 버튼은 감독관만 사용 가능
- 메시지별 paginationSessions로 리스트를 저장(재시작 시 세션 사라질 수 있어 재계산 로직 포함)

[3] 필요한 인텐트 추가
- GatewayIntentBits.GuildMembers 추가(전체 역할 보유자 조회에 필수)
*/