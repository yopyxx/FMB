...
// ✅ 서비스 계정 JSON 경로
const GOOGLE_KEYFILE = process.env.GOOGLE_KEYFILE || path.join(__dirname, 'service-account.json');
...
async function appendRowToSheet(rangeA1, values) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: 'USER_ENTERED',  // ✅ =IMAGE 같은 수식 사용 가능
    insertDataOption: 'INSERT_ROWS',
    requestBody: { values: [values] }
  });
}

// ✅ 시트 셀에 이미지로 표시 (원하면 HYPERLINK로 바꿀 수 있음)
...
client.once('ready', async () => {
  ...
  if (!fs.existsSync(GOOGLE_KEYFILE)) {
    ...
    console.log('   service-account.json을 프로젝트 폴더에 두거나 GOOGLE_KEYFILE 환경변수를 설정하세요.');
  }
...
client.on('interactionCreate', async interaction => {
  ...
  if (cmd === '소령행정보고' || cmd === '중령행정보고') {
    ...
    // ✅ 1) 닉네임: 실제 닉(displayName)으로 저장/시트 기록
    ...
    u.nick = displayName; // ✅ 로컬에도 displayName 저장
    ...
    // ================== ✅ 구글 시트 저장 (원자료 총점 = 행정건수 + 추가점수) ==================
    // ✅ 2) 총점수: 원자료(행정건수 + 추가점수)
    const totalScoreRaw = adminCount + extra;

    try {
      const photoUrls = photoAttachments.slice(0, 10).map(a => a.url);
      const photoCells = photoUrls.map(u => toImageFormula(u));
      ...
      if (is소령) {
        ...
        await appendRowToSheet('중령!A:Z', [
          ...
          input.서버역할,
          input.감찰,
          adminCount,        // 총행정건수(가중치 적용된 calculate중령 결과)
          input.인게임시험,
          input.코호스트,
          input.피드백,
          totalScoreRaw,     // ✅ 원자료 총점
      ...
      // 시트 실패해도 디스코드 답변은 정상 진행
      replyText += `\n\n⚠️ (참고) 구글 시트 저장에 실패했습니다. 콘솔 로그를 확인하세요.`;
    }
...
[구글 시트 저장]
- 닉네임: <@id>가 아니라 member.displayName (interaction.member.displayName) 저장
- 총점수: 원자료 (행정건수 adminCount + 추가점수 extra) 저장
- 사진: =IMAGE("url") 형태로 셀에 바로 이미지 표시(최대 10장)

필수 준비
1) npm i googleapis
2) service-account.json을 프로젝트 폴더에 두기 (또는 GOOGLE_KEYFILE 환경변수로 경로 지정)
3) 스프레드시트 공유에 서비스계정 이메일을 "편집자"로 추가
4) 시트 탭 이름을 '소령', '중령'으로 맞추기 (다르면 appendRowToSheet range 수정)
