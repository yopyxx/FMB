 ...
 *    ✅ Server Members Intent ON  (가입일/역할/role.members 조회에 필요)
 *
 * 3) 봇 초대 시 scope:
 *    ✅ bot
...
const CONFIG = {
  ...
  CLIENT_ID: process.env.CLIENT_ID,                 // ✅ 반드시 필요
  ...
  INCLUDE_ROLE_ID: '1018195906807480402',           // (점수표 표시 포함 대상)
  DEMOTION_EXCLUDE_ROLE_ID: '1477394729808298167',  // (강등 대상 제외)
...
if (!CONFIG.CLIENT_ID) {
  console.log('❌ CLIENT_ID가 설정되지 않았습니다! (환경변수 CLIENT_ID=봇 애플리케이션 ID)');
  process.exit(1);
}
...
function loadData() {
  ...
  if (fs.existsSync(CONFIG.DATA_FILE)) {
    try {
      ...
      console.log('⚠️ 데이터 파일 파싱 실패. 새로 생성합니다.');
      saveData();
    }
  } else {
    saveData();
  ...
  for (const key of ['소령', '중령']) {
    if (!data[key].history) data[key].history = { daily: {}, weekly: {} };
    if (!data[key].history.daily) data[key].history.daily = {};
    if (!data[key].history.weekly) data[key].history.weekly = {};
    if (!data[key].users) data[key].users = {};
    if (!data[key].lastWeekStart) data[key].lastWeekStart = '';
    if (!data[key].weekStart) data[key].weekStart = '';
  }
}
...
/* =========================
 * 4) TIME UTILS (reportDate: 02:00 기준)
 * ========================= */
...
function buildRoster(rankName, includeNickMap) {
  ...
  // include-role 보유자도 표시(보고 미제출 시 0점)
...
function embedDemotion(rankName, weekStart, weekEnd, listPage, page, totalPages, totalCount) {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      ...
      `**대상 인원**: ${totalCount}명\n` +
      `**제외**: 가입 7일 미만, 역할(${CONFIG.DEMOTION_EXCLUDE_ROLE_ID})\n\n` +
      lines
    )
    .setFooter({ text: `페이지 ${totalPages === 0 ? 0 : page + 1}/${Math.max(totalPages, 1)}` });
}
...
async function registerCommandsREST() {
  ...
  try {
    ...
    console.log('✅ 슬래시 명령어 등록 완료 (REST applicationGuildCommands)');
  } catch (e) {
...
async function replyPaginatedDemotions(interaction) {
  ...
  const rowMaj = makeNavRow('demote:maj', pMaj, pagesMaj.length);
  const rowLt = makeNavRow('demote:lt', pLt, pagesLt.length);
...
async function handleButton(interaction) {
  ...
  if (!state) return interaction.reply({ content: '⚠️ 페이지 정보가 만료되었습니다. 명령어를 다시 실행해 주세요.', ephemeral: true });
 ...
 * 15) COMMAND HANDLERS
 * ========================= */
function getGroup(rankName) {
  return rankName === '소령' ? data.소령 : data.중령;
}
...
async function handleReport(interaction, rankName) {
  ...
  // 누적은 편의상 올리되, 초기화/삭제 후 recomputeTotals로 정합성 맞춤
...
client.once('ready', async () => {
  ...
  // ✅ 안정적인 REST 등록 (강등대상 안 뜨는 문제 해결 핵심)
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    // Role restrictions for report commands
    ...
    if (cmd === '소령지난주점수' || cmd === '중령지난주점수') {
      ...
      const list = (weeklySnap.list || []).slice(0, CONFIG.PAGE_SIZE);
    ...
    if (cmd === '어제점수') {
      ...
      const mk = (rankName, date, snap) => {
        ...
        return new EmbedBuilder().setTitle(`${rankName} ${date} 점수`).setDescription(lines);
      };

      return interaction.reply({
        embeds: [header, mk('소령', yMaj.date, yMaj.snap), mk('중령', yLt.date, yLt.snap)],
      });
    }
    ...
    if (cmd === '지난주점수') {
      ...
      return interaction.reply({ embeds: [header, mk('소령', wMaj), mk('중령', wLt)] });
    }
    ...
    if (cmd === '소령오늘초기화' || cmd === '중령오늘초기화') {
      ...
      if (isAll) {
        ...
        recomputeTotals(group);
        saveData();
        return interaction.reply({ content: `✅ ${rankName} 오늘(${date}) 기록 전체 초기화 완료 (${cleared}명)`, ephemeral: false });
      }
      ...
      recomputeTotals(group);
      saveData();
      return interaction.reply({ content: `✅ ${targetUser} 님의 ${rankName} 오늘(${date}) 기록을 초기화했습니다.`, ephemeral: false });
    }
    ...
    if (cmd === '행정통계') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          ...
          `※ 오늘/주간 점수 표시에는 역할(${CONFIG.INCLUDE_ROLE_ID}) 보유자도 포함됩니다.`
        );
    ...
    // fallback
    return interaction.reply({ content: '⚠️ 알 수 없는 명령어 처리.', ephemeral: true });

  } catch (err) {
    console.log('❌ interaction 처리 오류:', err?.stack || err);
    if (interaction && !interaction.replied) {
      return interaction.reply({ content: '❌ 처리 중 오류가 발생했습니다. 콘솔 로그를 확인해 주세요.', ephemeral: true }).catch(() => null);
    }
  }
});
