...
// ================== 주간(일요일 02시 기준) 유틸 ==================
// getReportDate() 자체가 "02시 이전이면 전날"로 보정되므로,
// 여기서는 "해당 reportDate가 속한 주의 일요일"만 계산하면 됨.
function getSundayWeekStart(dateStr) {
  // KST 날짜로 안전하게 요일 계산: dateStr의 00:00을 KST(+09:00)로 해석
  const d = new Date(`${dateStr}T00:00:00+09:00`);
  const day = d.getUTCDay(); // 0=일,1=월,... (오프셋 포함되어 KST 요일과 일치)
  return addDays(dateStr, -day);
}

// ================== 누적 정합성(중요) ==================
// totalAdmin/totalExtra는 daily 기록의 합계이므로,
// 초기화/삭제/수정 시 불일치를 막기 위해 재계산 함수 제공
...
// ================== /초기화주간 핵심 로직 ==================
// "명령어 실행일(today) 기준 이전 7일" 중에서,
// "해당 주 일요일 02시 이후(=이번 주 구간)"는 보호하고,
// 보호구간 이전에 걸리는 날짜만 삭제한다.
// 삭제 범위: [today-7, thisWeekStart)  (thisWeekStart=이번 주 일요일)
...
// (주의) 자동 초기화는 범위 갱신/스냅샷 용도로 유지.
// 실제 데이터 삭제는 /초기화주간에서만 수행(운영 정책상 안전)
...
client.on('interactionCreate', async interaction => {
  ...
  if (cmd === '소령행정보고' || cmd === '중령행정보고') {
    ...
    // ✅ 누적은 유지하되, 초기화/삭제 시엔 recomputeTotals로 정합성 맞춤
  ...
  // ✅ /초기화주간:
  // "명령어 사용한 해당 주"에서 "일요일 02시 이후(보호)"를 제외하고
  // 그 이전 7일(rolling 7일 중 보호구간 이전에 걸리는 날짜만) 점수를 삭제
  if (cmd === '초기화주간') {
    const today = getReportDate();
    const thisWeekStart = getSundayWeekStart(today);
    ...
    // 표시용: rangeEnd는 일요일(미포함)이므로 -1일을 표시
    const endShown = addDays(majRes.rangeEnd, -1);

    if (majRes.nothingToClear && ltRes.nothingToClear) {
      return interaction.reply({
        content:
          `ℹ️ 초기화할 구간이 없습니다.\n` +
          `- 오늘: ${today}\n` +
          `- 보호(이번 주): ${thisWeekStart} ~ 현재 (일요일 02시 이후)\n` +
          `- 지난 7일이 전부 보호 구간에 포함되어 삭제 대상이 0일입니다.`,
        ephemeral: false
      });
    }
    ...
    return interaction.reply({
      content:
        `🔄 주간 초기화 완료 (Rolling 7일, 보호구간 제외)\n` +
        `- 오늘: ${today}\n` +
        `- 보호(이번 주): ${thisWeekStart} ~ 현재 (일요일 02시 이후)\n` +
        `- 삭제 구간: ${majRes.rangeStart} ~ ${endShown}\n` +
...
================== 꼼꼼 점검 메모(핵심) ==================
1) /초기화주간 원인:
   - 기존은 weekStart만 갱신해서 "원자료(daily)"가 남아 점수가 안 없어짐.
   - 현재는 daily의 해당 날짜를 실제 삭제해서 점수가 확실히 초기화됨.

2) 누적(totalAdmin/totalExtra) 정합성:
   - daily 삭제/초기화 후 누적이 남아있으면 통계가 틀어짐.
   - recomputeTotals(group)로 항상 맞춤.

3) 날짜:
   - getReportDate()가 "02시 이전이면 전날" 처리.
   - weekStart(일요일)는 getSundayWeekStart()로 계산.
   - addDays()는 UTC 기반이지만, dateStr 자체가 YYYY-MM-DD라 비교/슬라이스 용도로 안정.

4) 주간 자동 초기화 스케줄:
   - 일요일 02:00으로 변경(요구 기준과 일치).
   - 자동은 스냅샷/범위 갱신만, 실제 삭제는 수동 /초기화주간에서만.

5) 퍼센타일:
   - Math.ceil(rank/n*100) 방식이라 n이 작으면 1등도 34% 같은 값이 나올 수 있음.
   - "상위 X%" 표기의 정의를 바꾸고 싶으면 퍼센타일 정의를 재설계해야 함.

6) 권한:
   - SUPERVISOR_ROLE_IDS 다중 지원 OK.
   - 보고 명령은 소령/중령 역할 제한 OK.

7) 안전:
   - 파일 저장 시 DATA_DIR 보장 추가.
*/