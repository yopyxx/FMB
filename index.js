 ...
 *
 * ✅ 주요 수정/점검 포인트(명령어 일부가 “안 되는” 증상 대응)
 * 1) 무거운 명령(멤버 전체 fetch/주간 합산/강등대상)은 3초 제한 때문에 “Unknown interaction”으로 실패할 수 있음
 *    → 해당 명령은 interaction.deferReply() 후 editReply()로 응답하도록 수정
 * 2) 페이지네이션(28명 초과) 버튼 지원: 오늘/주간/소령어제/중령어제/소령지난주/중령지난주
 *    → 버튼은 “명령 실행자만” 클릭 가능
 * 3) (A) /어제점수, /지난주점수는 첫 페이지(최대 28명)만 표시 + 안내 문구
 * 4) 임베드 description 4096자 제한 방어(닉네임/문장 트림)
 * 5) “명령어가 사용 안 됨” 진단을 위해 cmd별 로그 강화 + 알 수 없는 명령 방어
 */
...
// ================== 페이지네이션 캐시 ==================
const PAGE_CACHE = new Map(); // key -> { kind, rankName, meta, items, createdAt, ownerId }
const PAGE_CACHE_TTL_MS = 15 * 60 * 1000; // 15분
const PER_PAGE = 28;
...
function buildPagerRow(cacheKey, page, totalPages) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`pg|${cacheKey}|1`)
      .setLabel('⏮ 처음')
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(page <= 1),

    new ButtonBuilder()
      .setCustomId(`pg|${cacheKey}|${page - 1}`)
      .setLabel('◀ 이전')
    ...
    new ButtonBuilder()
      .setCustomId(`pg|${cacheKey}|${totalPages}`)
      .setLabel('마지막 ⏭')
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(page >= totalPages),
  );
...
async function buildRosterForRole(guild, includeRoleId, excludeRoleIds) {
  if (!guild) return [];
  await guild.members.fetch(); // 전체 fetch (무거움)
...
function createDailyEmbedPaged(rankName, title, dateStr, pageSlice, page, totalPages, totalUsers) {
  ...
  return new EmbedBuilder()
    .setTitle(title)
    .setDescription(desc)
    .setFooter({ text: `페이지 ${page}/${totalPages} • 총 ${totalUsers}명 • 최소업무 미달자는 0점 + 퍼센타일 산정 제외` });
}
function createWeeklyEmbedPaged(rankName, title, weekStart, weekEnd, pageSlice, page, totalPages, totalUsers) {
  ...
  return new EmbedBuilder()
    .setTitle(title)
    .setDescription(desc)
    .setFooter({ text: `페이지 ${page}/${totalPages} • 총 ${totalUsers}명 • 주간=일~토(7일) 합산` });
}

// (A) 공용 명령(/어제점수,/지난주점수)용: 첫 페이지만(최대 28명) 표시
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    console.log(`▶ CMD: ${cmd} by ${interaction.user?.id}`);
    ...
    // ================== (중요) 무거운 명령은 deferReply로 타임아웃 방지 ==================
    ...
    if (heavyCommands.has(cmd)) {
      // 이미 replied/deferred가 아니라면 defer
    ...
    if (cmd === '초기화주간') {
      ...
      // 초기화주간은 무겁지 않아서 defer 안 했을 수도 있음 → replied 체크
      const payload = {
        content:
    ...
    if (cmd === '강등대상') {
      ...
      const weekStartMaj = data.소령.weekStart || thisWeekStart;
      const weekStartLt = data.중령.weekStart || thisWeekStart;
      ...
      const lines = targets.length
        ? targets.slice(0, 40).map((t, i) => {
            ...
            const nick = trunc(t.nick, 24);
            return `**${i + 1}.** [${t.rankLabel}] ${nick} ${mention} — **${t.total}점**`;
          }).join('\n')
        : '✅ 조건에 해당하는 인원이 없습니다.';
    ...
    // ================== 여기까지 왔는데 처리 못한 cmd면 안내 ==================
    // (명령어 “몇 개가 사용 안 됨”을 잡기 위한 안전장치)
    if (!interaction.replied && !interaction.deferred) {
      return interaction.reply({ content: `ℹ️ 알 수 없는 명령어: ${cmd}`, ephemeral: true });
    }
    if (interaction.deferred && !interaction.replied) {
      return interaction.editReply({ content: `ℹ️ 알 수 없는 명령어: ${cmd}` });
    }

  } catch (e) {
    console.error('❌ interactionCreate 처리 중 오류:', e);
    try {
      console.error('❌ kind=', interaction?.type, 'cmd=', interaction?.commandName, 'user=', interaction?.user?.id);
    } catch (_) {}
